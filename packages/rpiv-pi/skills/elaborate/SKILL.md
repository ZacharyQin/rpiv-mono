...
- **Never carry the plan's trailing `## Whole-Plan Verification` block into your elaboration doc.** No phase owns it; the stitch strips it from the last phase's span and re-emits it itself (authored verbatim, or derived from the AV items) — a lane that spliced it back in would duplicate the block.
