...
The graph is validated at load time. `validateWorkflow()` rejects a dangling edge, a `produces` stage with no outcome (neither wired inline nor derivable from the skill's contract), or a `reads:` name no stage publishes. It also warns when a `gate` routes on a stage whose data has no schema to validate against, inline or contract-sourced. All of this happens before a single session opens. A broken workflow fails to load, not halfway through a run. `/wf <name>` previews the graph so you can read every stage, its dispatch, and where the branches land before you spend a token.
