...
## Dependencies
- **`sherpa-onnx-node@1.13.0`** (CJS, no upstream `.d.ts`): `OfflineRecognizer` — created async (`createAsync`), decoded async (`decodeAsync`) behind a per-engine FIFO queue
...
## STT Engine Lifecycle (one stream per utterance, async decode)
Recognition is per-utterance: a fresh stream is opened, samples are decoded, the stream is released. The engine is created via `OfflineRecognizer.createAsync` and decodes via `await recognizer.decodeAsync(stream)` under a FIFO per-engine decode queue (stt-engine.ts) — synchronous decode blocks the event loop, so all recognitions serialize through the queue. The native handle has no destructor — `release()` is a stable-contract no-op kept so callers can write symmetric setup/teardown. Sample-rate must match the mic-source contract.
...
## Error/Diagnostic Sinks (TUI-safe)
Every audio-layer error path is funnelled through an append-only file sink; a sibling `appendDiagnosticLog` records non-error breadcrumbs (e.g. decode latency). The sinks **never throw** — `stderr` writes would corrupt the TUI render, so failure of the sink itself is silently swallowed.
